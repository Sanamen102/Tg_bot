...
class Settings(BaseSettings):
    ...
    awg_check_host: str = ""
    # Сколько циклов мониторинга подряд туннель должен молчать, прежде чем
    # алертить (защита от коротких морганий; при интервале 15 мин 2 = ~30 мин)
    awg_confirm_fails: int = 2
    ...
    # --- Библиотека книг (Kavita) ---
    # Куда складывать книги, присланные файлом в чат (путь внутри контейнера).
    # Пусто = приём книг выключен.
    books_dir: str = "/media/books"

    # --- Скачивание видео по ссылке (yt-dlp) ---
    # Куда сохранять при выборе «В медиатеку» (путь внутри контейнера бота,
